...
def compute_word_embedding(span, text_emb):
    ...
    for token_id in span:
        # vectors.append(torch.mean(text_emb[token_id], dim=0))
        vectors.append(torch.mean(text_emb[token_id][-4:], dim=0))
...
class Encoder:
    ...
    def get_mentions_spans(self, ids, context_span=False):
        """
        ...
        try:
            while it < len(ids):
                if ids[it] == s_tag:
                    ...
                    if context_span:
                        # Appending the preceding context
                        for pid in range(it-1, 0, -1):
                            if ids[pid] == e_tag or ids[pid] == s_tag:
                                continue
                            p_context = [pid] + p_context
                            if len(p_context) == 31:
                                break

                    while ids[it-1] != e_tag:
                        m_span.append(it)
                        it += 1
                        if it == len(ids):
                            last = False
                    ...
                    if last:
                        if context_span:
                            # Appending the succeeding context
                            for sid in range(it+1, it+33):
                                if sid == len(ids):
                                    break
                                if ids[sid] == e_tag or ids[sid] == s_tag:
                                    continue
                                s_context.append(sid)
                                if len(s_context) == 31:
                                    break

                            context_span = p_context + m_span + s_context
                            m_span = context_span
            ...
            print(f"Index Error: {ids[:10]}")
            pass

        return mentions

    def embed_mentions(self, corpus, ordered_golden_entities, ordered_mentions, with_context=False):
        """
        ...
        for text_id in list(corpus.keys()):
            ...
            for chunk in range(len(tokenized_text)):
                ...
                mentions_spans = self.get_mentions_spans(indexed_tokens, with_context)
                mspans = self.get_mentions_spans(indexed_tokens, False)

                # Segment tensor
                # TODO: change to zeroes and see what happens
                segments_tensor = torch.zeros(256)
                ...
                with torch.no_grad():
                    ...
                    for m_span in mentions_spans:
                        if len(mentions_spans) != tokenized_text[chunk].count("[m]"):
                            print("HAAAAAAAAAAAALLLLLLLLLLTTTTTTTTTTtt")
