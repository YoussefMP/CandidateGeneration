...
#    file = open(f"{fman.results_repo}/final_trained_blink_emb_21_11_22.tsv", "r", encoding="utf-8")
...
#    urls_file.close()
    ...
    HNSW.serialize(f"{fman.results_repo}/final_trained_blink_emb_21_11")
...
def eval_model(encoder, dataloader, f_idx, url_map, set_size, fman):
    ...
    with open(f"{fman.models_repo}/BLINK_output/Eval/final_trained_results_{test_file}.json", "w", encoding="utf-8") as of:
...
if __name__ == "__main__":
  ...
  if not os.path.exists(f"{file_manager.results_repo}/final_trained_blink_emb_21_11"):
      ...
      HNSW = DenseHNSWFlatIndexer(1024, store_n=64, ef_search=32, ef_construction=32)
      HNSW.deserialize_from(f"{file_manager.results_repo}trained_blink_emb_21_11")

  urls_file = open(f"{file_manager.results_repo}trained_blink_url_map_20_11_22.pkl", "rb")
  ...
  reranker = load_model(params, f"{model_path}/BLINK_output/epoch_6", model_name)
