...
#            if len(list(mentions_dict.keys())) == 20:
#                break
    ...
    def load_specific_data(self, file_path, top=0):
        ...
        if top != 0:
            ...
            count = dict(list(count.items())[-50:])
        ...
        for row in csv_reader:
            if len(row) > 1:
                if row[2] in count.keys() and count[row[2]] > 2:
                    txt_id = row[0]
                    mention = row[2]
                    entity = row[1]
                    m_emb = str_to_tensor(row[3], 768)
                    ...
                    train_dataset.append((str_to_tuple(txt_id), mention, entity, m_emb, e_emb))
