...
class BiEncoderModule(torch.nn.Module):
    def __init__(self, params):
        ...
        self.ctxt_bert = BertModel.from_pretrained(params["bert_model"])
        self.cand_bert = BertModel.from_pretrained(params['bert_model'])

        # Initializing the context encoder
        ...
        # Initializing the candidates encoder
        ...
        self.config = self.ctxt_bert.config

    def forward(self, tokens_ids, segment_idx_ctxt, mask_ctxt, entity_embeddings, segment_idx_cands, mask_cands):

        embedding_ctxt = None
        if tokens_ids is not None:
            embedding_ctxt = self.context_encoder(
                tokens_ids, segment_idx_ctxt, mask_ctxt
            )

        embedding_cands = None
        if entity_embeddings is not None:
            embedding_cands = self.cand_encoder(
                None, segment_idx_cands, mask_cands, inputs_embeds=entity_embeddings
            )
